use crate::app::{BIBLE_OPTIONS, CharistApp, Message, VersePopup};
...
pub(crate) fn view(app: &CharistApp) -> Element<'_, Message> {
    ...
    if let Some(link) = &app.open_footnote {
        stack![base, app.view_footnote_popup(link)].into()
    } else {
        base
    }
}
    ...
    fn verse_popup_content(&self) -> Element<'_, Message> {
        let content: Element<'_, Message> = match &self.verse_popup {
            Some((_, VersePopup::Menu)) => column![
                button::text("Copy").on_press(Message::CopySelection),
                button::text("Add bookmark").on_press(Message::AddBookmark),
                button::text("Add bookmark with note").on_press(Message::OpenNoteInput),
            ]
            .spacing(4)
            .into(),
            Some((_, VersePopup::Note(text))) => column![
                text_input("Note", text).on_input(Message::NoteTextChanged),
                row![
                    button::suggested("Save").on_press(Message::SaveNoteBookmark),
                    button::standard("Cancel").on_press(Message::CloseVersePopup),
                ]
                .spacing(8),
            ]
            .spacing(8)
